...
def active_alarms(): # Kontrollerar om larm är aktiva samt visar vilka larm som är aktiva.
    if not cpu_lista and not ram_lista and not disk_lista:
        print("Inga larm är aktiva.")
    else:
        print(f"Alarmen visas i % och skiljs på meed ','. Tomma '[]' betyder att det inte finns något larm av den typen.")
        print(f"CPU-larm satt till: {cpu_lista}%")
        print(f"RAM-larm satt till: {ram_lista}%")
        print(f"Disk-larm satt till: {disk_lista}%")
